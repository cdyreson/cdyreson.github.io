 ...
 *
 *  $Id: DLNBase.java 9474 2009-07-23 20:12:38Z dizzzz $
 */
package xmorph.org.exist.numbering;
...
import xmorph.org.exist.util.ByteConversion;
import xmorph.org.exist.storage.io.VariableByteInput;
 ...
 *
 */
public class DLNBase implements Serializable {

    // Added: Ryan Grapp
    protected int schemaTypeID;
    /**
    ...
    /**
     * Is this one equal to the other to the given level in the tree?
     * Used to check if they have a common lca at this level (or below)
     * @return whether they are equal
     */
    public boolean hasCommonLCA(DLNBase other, int level) {
     ...
     */
    public int size() {
        return bits.length + NodeId.LENGTH_SCHEMA_TYPE_ID;
    }
    ...
    /**
     * Return all level ids converted to int. Had a bug
     * so rewrote it, probably don't use this one.
     *
     * @return all level ids in this node id.
     */
    ...
    public void serialize(byte[] data, int offset) {
        ...
        ByteConversion.intToByte(schemaTypeID, data, offset + bits.length);
    }

    public static int getLengthInBytes(int units, byte[] data, int startOffset) {
        return (int) Math.ceil(units / 8.0) + NodeId.LENGTH_SCHEMA_TYPE_ID;
    }
...
//    }
    ...
    /**
     * Curtis added this.  Checks to see if the current DLN
     * is equal to the previous up to the indicated number of bits.
     * returns true/false.  This should be startsWith(other, bitCount).
     * 
     * @param other
     */
    public boolean compareBitsExact(DLNBase other, int bitCount) {
        int bytes = bitCount / 8;
        int remaining = bitCount % 8;
        for (int i = 0; i < bytes; i++) {
            if (bits[i] != other.bits[i]) {
    ...
    public String toString() {
        ...
        while (offset <= bitIndex) {
            ...
            if (offset > bitIndex) return buf.toString();
            int id = getLevelId(offset);
            buf.append(id);
            offset += getUnitsRequired(id) * BITS_PER_UNIT;
        }
