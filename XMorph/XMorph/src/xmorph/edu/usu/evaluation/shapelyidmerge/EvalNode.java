...
public class EvalNode {
    ...
    public EvalNode(ShapeNode shapeNode, Database db, boolean parentFreeMe) {
        ...
        if (shapeNode.getBaseTypeId() == null) {
            ...
            idIterator = new CachedIterator(db.typedNodeTableCollection.iterator(shapeNode.getBaseTypeId()), shapeNode);
            //System.out.println("EVALNODE shapenode " + db.typesLookupTable.get(shapeNode.getBaseTypeId()) + " " + shapeNode.getBaseTypeId().getName() + " " + shapeNode + " " + shapeNode.isForceAttribute());
        }
    ...
    public Node kickStartRoot() {
        ...
        // Fetch the next node
        Id currentId = idIterator.next().getId();
    ...
    public boolean evaluate(Node currentNode, Graph graph) {
        ...
        while (childrenIterator.hasNext()) {
            ...
            ShapeNode childShapeNode = shapelyId.getShape();
            //System.out.println("child shape node " + childShapeNode + " " + childShapeNode.imageXML());
            ...
            // Should child be an attribute? 
            if (childShapeNode.isForceAttribute()) {
                //System.out.println("child forcing attribute");
                node.setAttribute();
            }

            // Should child be an element? 
            if (childShapeNode.isForceElement()) {
                node.clearAttribute();
            }
