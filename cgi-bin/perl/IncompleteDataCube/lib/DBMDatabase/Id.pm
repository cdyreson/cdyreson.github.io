...
=head2 fromStringNoCreate(string $string)
...
Create an Id from a string, but do not create the Id if it does
not already exist!  This will return false if it could not 
create the Id.

=cut

#---------------------------------------------------------------------
sub fromStringNoCreate {
  my ($s) = @_;
  return Convert::toIdNoCreate($s);
  }
#---------------------------------------------------------------------
