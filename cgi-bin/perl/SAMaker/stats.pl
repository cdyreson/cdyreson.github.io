...
sub starter {
  ...
  while (<$path/log*>) {
    ...
    open (OLOG, ">>$path/alllog") || &internalError("could not open alllog\n");
    # iterate through the log
    while (<LOG>) {
      my $line = $_; 
      print OLOG "$fileName $line";
      chomp $line;
      if ($line !~ /\d+([^\d])/) {next;}
      my $sep = $1;
      ...
      $$questionref{'TOTALRIGHT'}= 0 unless defined $$questionref{'TOTALRIGHT'};
      $$questionref{'TOTAL'} = 0 unless defined $$questionref{'TOTAL'};
      $$questionref{$action} = 0 unless defined $$questionref{$action};
      if ($action eq 'INCORRECT') { 
        ...
        $$questionref{'WRONG'}->{$answer} = 0 
          unless defined $$questionref{'WRONG'}->{$answer};
        $$questionref{'WRONG'}->{$answer} =  
          $$questionref{'WRONG'}->{$answer} + 1;
        }
      elsif ($action eq 'CORRECT') { 
        $$questionref{'TOTAL'} = $$questionref{'TOTAL'} + 1;
        $$questionref{'RIGHT'} = {} unless defined $$questionref{'RIGHT'};
        my $answer = join("\n",@items);
        $$questionref{'RIGHT'}->{$answer} = 0 
          unless defined $$questionref{'RIGHT'}->{$answer}; 
