...
# set up the global state
 ...
 $path = q{C:\Inetpub\wwwroot\CDyreson\prweb\320\lessons\ };
 $path = &cleanWS($path) . $rest;
 #my $statspath = q{C:\Inetpub\wwwroot\CDyreson\prweb\320\stats\} . $rest;
...
sub openStats {
  my ($userid) = @_;
  my $logfile = "$path" . &cleanWS('\ ') . "log$userid";
