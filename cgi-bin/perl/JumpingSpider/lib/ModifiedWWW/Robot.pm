...
ModifiedWWW::Robot - configurable web traversal engine (for web robots & agents)

=head1 SYNOPSIS

   use ModifiedWWW::Robot;
   
   $robot = new ModifiedWWW::Robot('NAME'     => 'MyRobot',
			   'VERSION'  => '1.000',
...
package ModifiedWWW::Robot;
require 5.002;
use strict;

use HTTP::Request;
use HTTP::Status;
use ModifiedHTML::Parse;
use URI::URL;
use LWP::RobotUA;
use IO::File;
...
Builds on standard Perl 5 modules for ModifiedWWW, HTTP, ModifiedHTML, etc.
...
(
 ...
 'STARTURL',      	    'the url to start at',
 'SPILLFILE',      	    'name of the file for temporay work',
 'MAXPAGES',      	    'maximum number of pages/cut out after this',
...
=head1 CONSTRUCTOR
   ...
   $robot = new ModifiedWWW::Robot( <attribute-value-pairs> );
...
sub run
{
    ...
    my $noindex;
    my $nofollow;
    # do 1500 pages or none
    my $count = $self->{'MAXPAGES'} || 1500;
	...
	if ($response->is_error)
	{
	    $self->invoke_hook_procedures('invoke-on-get-error',
					   $url, $response);
	    undef $structure if defined $structure;
	    next;
	}
        ...
        if ($response->base ne $url) {
          $base = new URI::URL($response->base);
          }
        else {  
          $base = $url;
          }
	...
	#---------------------------------------------------------------
        if ($response->content_type eq 'text/html')
        {
            ...
            else { print "NOFOLLOW " . $url->as_string . "\n"; }
        }
	if ($noindex == 0)
	{
        ...
        else { print "NOINDEX " . $url->as_string . "\n"; }
	...
	{
            my ($link_url, $text) = @$_;
	    $self->invoke_hook_procedures('invoke-on-link', $url, $link_url, $text);
	    $self->addUrl($link_url->as_string);
	}
      $count--;
      if ($count <= 0) { $self->spill(); return; } 
    }
    continue
    {
	#------------------------------------------------------------------
...
sub extract_links
{
    ...
    # Ignore Index of pages
    return () if $$structure{'title'} =~ /^\s*Index of \//;

    foreach (@{ $$structure{'links'} }) {
        my ($link, $text) = @$_;
	...
	#---------------------------------------------------------------
	# add a slash to it if it doesn't end in html
	# ignore the link if it ends in something bad
	#---------------------------------------------------------------
        if ($link =~ m![^/.]+\.([^/.]+)$!) {
          my ($extension) = $1;
          if ($extension =~ /^gz$/i) { next; }
          elsif ($extension =~ /^ps$/i) { next; }
          elsif ($extension =~ /^gif$/i) { next; }
          elsif ($extension =~ /^jpg$/i) { next; }
          elsif ($extension =~ /^png$/i) { next; }
          elsif ($extension =~ /^xbm$/i) { next; }
          elsif ($extension =~ /^wav$/i) { next; }
          elsif ($extension =~ /^mpg$/i) { next; }
          elsif ($extension =~ /^zip$/i) { next; }
          elsif ($extension =~ /^pdf$/i) { next; }
          elsif ($extension =~ /^txt$/i) { next; }
          elsif ($extension !~ /^s?[hH][tT][mM][lL]?$/) { 
            $link .= '/';
            }
          }
	...
	#---------------------------------------------------------------
        ...
        $link_url = new URI::URL($link, $base);
...
#	$structure - an ModifiedHTML::Element object for the page to check
#
...
sub check_protocol
{
    ...
    #-------------------------------------------------------------------
    $nofollow = $$structure{'nofollow'};
    $noindex = $$structure{'noindex'};
...
#=======================================================================
# spill() - Shut down the robot, spill the data in it to a file
#=======================================================================
sub spill
{
    my $self       = shift;
    my $filename   = $self->{'SPILLFILE'};

    open(FILE, ">$filename") || die "could not open $filename in spill\n";
    #my %joe = $self->{'SEEN_URL'};
    foreach (keys %seenUrl) { print FILE "$_\n"; }
    print FILE "\f\n";
    foreach (@{ $self->{'URL_LIST'} }) { print FILE "$_\n"; }
    close FILE;
}

#=======================================================================
# unspill() - Read the state back in and restart the robot
#=======================================================================
sub unspill
{
    my $self       = shift;
    ...
    if (! -e $filename) {
      $self->{'URL_LIST'} = [];
      push @{ $self->{'URL_LIST'} }, $self->{'STARTURL'};
      return;
      }
    ...
    while (<FILE>) {
      chomp;
      $url = $_;
      if ($url =~ /^\f$/) { $list = 0; next; }
      if ($list) {
        $seenUrl{$url} = 1;
        }
      else {
        push @{ $self->{'URL_LIST'} }, $url;
        }
      }
    close FILE;
}
...
sub set_attribute
{
	...
	#---------------------------------------------------------------
	$ModifiedHTML::Parse::IGNORE_TEXT = $new_value;
    }
...
sub addUrl
{
	...
	{
	    $urlObject = $url;
            die $url->abs() . "should never get here!!!!";
	}
        else
	{
	 ##   $urlObject = new URI::URL($url);
	    #z$urlObject = eval { new URI::URL($url) };
	    #zif ($EVAL_ERROR)
	   #z {
	#z	$self->warn("addUrl() unable to create URI::URL object",
         #z                   "URL:   $url",
          #z                  "Error: $EVAL_ERROR");
#z		$status = 0;
#z		next;
...
sub next_url
{
    ...
    #print "rem " . scalar @{ $self->{'URL_LIST'} } . "\n";
    if ($self->{'TRAVERSAL'} eq 'depth')
    {
        my $urlObject = new URI::URL(pop @{ $self->{'URL_LIST'} });
        return $urlObject;
	#zreturn pop @{ $self->{'URL_LIST'} };
    }

    my $urlObject = new URI::URL(shift @{ $self->{'URL_LIST'} });
    return $urlObject;
    #z return shift @{ $self->{'URL_LIST'} };
}
...
This section lists the hooks which are supported by the ModifiedWWW::Robot module.
...
ModifiedHTML::Element object which is the root of a tree structure constructed
...
B<text/html>
   ...
   $robot = new ModifiedWWW::Robot('NAME'     =>  'Validator',
			   'VERSION'  =>  1.000,
			   'EMAIL'    =>  'fred@foobar.com');
   ...
   sub follow_test {
      ...
      return 0 
        if $url =~ /\.(gz|ps|gif|jpg|png|xbm|wav|mpg|zip|pdf|txt)[/]?$/;
...
The Robot.pm module builds on a lot of existing Net, ModifiedWWW and other
Perl modules.
