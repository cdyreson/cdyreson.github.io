package ModifiedHTML::Parser;

# $Id: Parser.pm,v 2.6 1997/02/21 09:32:14 aas Exp $

=head1 NAME

ModifiedHTML::Parser - SGML parser class
	...
	# Then, finally we look for a start tag
		...
		if ($$buf =~ s|^>||) {
		    ...
		    $self->text($eaten);
                    ...
                    # CARRIAGE RETURN ADDS SOME SPACE!!!!!!!!!!!
...
sub start
{
    ...
    if ($tag eq 'a') {
      ...
      if (defined $$attr{'src'}) { 
        $linkURL = $$attr{'src'};
        $linkText = $$attr{'src'} . ' (frame) ';
        my $st = $self->{'_structure'};
        push @{ $$st{'links'} }, [$linkURL, $linkText];
        #print "adding frame $tag $linkURL $linkText \n";
        }
      }
    elsif ($tag eq 'title') {
      $wantTitleText = 1;
      ...
      return unless defined $$attr{'name'};
      return unless defined $$attr{'content'};
      return unless $$attr{'name'} =~ /robots/i;
      my $st = $self->{'_structure'};
      if ($$attr{'content'} =~ /nofollow/i) {
        $$st{'nofollow'} = 1;
        }
      if ($$attr{'content'} =~ /noindex/i) {
        $$st{'noindex'} = 1;
        }
      if ($$attr{'content'} =~ /none/i) {
        $$st{'nofollow'} = 1;
        $$st{'noindex'} = 1;
        }
      }
...
{
    my($self, $tag) = @_;
    if ($tag eq 'a' && $wantLinkText) {
      my $st = $self->{'_structure'};
      push @{ $$st{'links'} }, [$linkURL, $linkText];
      $wantLinkText = 0;
      }
    elsif ($tag eq 'h1' && $wantHeaderText) {
      my $st = $self->{'_structure'};
      push @{ $$st{'headers'} }, $headerText;
      $wantHeaderText = 0;
      }
    elsif ($tag eq 'title' && $wantTitleText) {
      my $st = $self->{'_structure'};
      $$st{'title'} = $titleText;
      $wantTitleText = 0;
      }
