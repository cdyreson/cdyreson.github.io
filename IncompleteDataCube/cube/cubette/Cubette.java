...
public class Cubette {
  ...
  /**
  * The constructor does nothing, all the important methods are static.
  **/
  public Cubette() {
    }
  ...
  **/
  private static void innerCrossProduct(
    ...
    if (current == measures.length) {
      ...
      Id[] newSoFar = new Id[sofar.length];
      for (int i = 0; i < sofar.length; i++) newSoFar[i] = sofar[i];
      Id[] newMeasures = new Id[measures.length];
      for (int i = 0; i < measures.length; i++) newMeasures[i] = measures[i];
      CubetteSpecification temp = new CubetteSpecification(newSoFar, newMeasures);
      //CubetteSpecification temp = new CubetteSpecification(sofar, measures);
  ...
  **/
  public static void increment(
    ...
    // exit if we've done this one already this round
    if (doneThisRound.containsKey(countList)) {
      return;
      }
